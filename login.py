...
def abrir_login():
    ...
    # Nomeando a janela
    janela_l.title("Login - Finance Manager")

    # Configurando o tamanho da janela e cor de fundo
    janela_l.geometry("800x600")
    janela_l.configure(bg="black")
    ...
    fundo = tk.Label(janela_l)
    fundo.place(x=0, y=0, relwidth=1, relheight=1)

    def ajustar_fundo(event=None):
        largura = janela_l.winfo_width()
        altura = janela_l.winfo_height()
    ...
    def confirmar_login():
        ...
        if not email or not senha:
            messagebox.showwarning("Campos obrigatorios", "Preencha email e senha.") #Caso falte algum dos campos a serem prenchidos
            return
    ...
    janela_l.bind("<Configure>", ajustar_fundo)

    # Frame central para manter os campos alinhados em qualquer tamanho de tela
    frame_login = tk.Frame(janela_l, bg="black", padx=35, pady=25)
    ...
    botao_login.pack(pady=(20, 8))

    botao_cadastro = tk.Button(
        frame_login,
        text="Nao tenho cadastro",
        font=("Arial", 12),
